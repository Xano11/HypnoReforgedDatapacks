...
tellraw @a[tag=towered] ["",{"selector":"@s","italic":true},{"text":" joined the queue!","bold":true,"color":"gold"},{"text":" Click here to join the queue! ","clickEvent":{"action":"run_command","value":"/execute if entity @s[tag=!partied] if score @e[limit=1,sort=nearest,type=minecraft:armor_stand,tag=elevator] elevator matches ..839 run function hypno:elevator_queue5"}},{"text":"[4/5]","underlined":true}]
